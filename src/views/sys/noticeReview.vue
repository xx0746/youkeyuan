<template>
  <div
      ...
      <el-tabs v-model="activeName" @tab-click="handleTabClick">
        ...
        <el-tab-pane name="2">
          ...
          <el-table
            ...
            <el-table-column align="center" label="附件">
              <template slot-scope="scope">
                <i v-if="scope.row.fileCount!=0" class="el-icon-download" @click="handleDownload(scope.row)"></i>
              </template>
            </el-table-column>
    ...
    <el-dialog title="公告详情" class="info-dialog" :visible.sync="dialogTableVisible">
      <show-info :data="tableInfo.sysNotice" :list="tableInfo.sysNoticeFiles" :options="option"></show-info>
    </el-dialog>
  </div>
</template>
<script>
...
import { getList, deleteItem, audit, turnReason, downLoad, deleteSystemNotice } from "@/api/noticelist";
export default {
  components: {
    showInfo
  },
  ...
  methods: {
    handDel(row) {
      this.$confirm("是否删除", "提示", {
        ...
        deleteSystemNotice(row.id).then(res => {
          if (res.data.code != "1") {
    ...
    // 预览
    openView(val) {
      systemNoticeDetail(val.id).then(res => {
        console.log(res);
        if(res.data.code == 1) {
